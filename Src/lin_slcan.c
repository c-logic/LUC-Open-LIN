...
uint8_t addLinMasterRow(uint8_t *line, uint8_t classicChecksum) {
	...
	if (line[1] == '2') { // reset schedule table
		open_lin_hw_reset();
		slcan_state = SLCAN_STATE_CONFIG;
		master_frame_table_size = 0;
		return 1;
	} else if (line[1] == '1') { // start sending
		if (lin_type == LIN_SLAVE) {
			open_lin_net_init((open_lin_frame_slot_t*) master_frame_table,
					master_frame_table_size);
		} else {
			open_lin_master_dl_init(master_frame_table,
					master_frame_table_size);
			//wakeUpLin();
		}
...
//    	diagnostic_slot = 1;
	...
	if ((line[0] == 'r') || (line[0] == 'R'))
		array_ptr->slot.frame_type = OPEN_LIN_FRAME_TYPE_RECEIVE, array_ptr->slot.checksum_type =
				classicChecksum ?
						OPEN_LIN_CHECKSUM_TYPE_CLASSIC :
						OPEN_LIN_CHECKSUM_TYPE_EXTENDED;
	else
		array_ptr->slot.frame_type = OPEN_LIN_FRAME_TYPE_TRANSMIT, array_ptr->slot.checksum_type =
				classicChecksum ?
						OPEN_LIN_CHECKSUM_TYPE_CLASSIC :
						OPEN_LIN_CHECKSUM_TYPE_EXTENDED;
	// data
	...
	if (line[0] < 'Z') // if upper case
			{
		if (!parseHex(&line[4], 2, &temp))
			return 0;
		array_ptr->offset_ms = temp;
		if (!parseHex(&line[6], 2, &temp))
			return 0;
...
void lin_slcan_rx_handler(t_open_lin_data_layer_frame *f) {
	lin_slcan_slot.pid = f->pid & OPEN_LIN_ID_MASK;
