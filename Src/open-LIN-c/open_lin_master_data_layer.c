...
l_bool open_lin_master_data_tx_data(open_lin_frame_slot_t *slot)
{
	...
	result = (open_lin_hw_tx_byte(open_lin_data_layer_checksum(slot->pid, slot->data_length, slot->data_ptr, slot->checksum_type))) && result;
	return result;
}
...
l_bool open_lin_master_dl_rx(l_u8 rx_byte)
{
	...
	if (lin_master_state == OPEN_LIN_MASTER_DATA_RX)
	{
		...
		{
			uint8_t checksum = open_lin_data_layer_checksum(slot->pid,slot->data_length,frame_tmp_buffer, slot->checksum_type);
			if (rx_byte == checksum)
			{
