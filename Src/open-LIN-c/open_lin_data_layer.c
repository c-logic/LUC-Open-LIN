...
open_lin_checksum_t open_lin_data_layer_checksum(open_lin_pid_t pid, l_u8 lenght, const l_u8* data_ptr, open_lin_checksum_type_t cs_type) {
	l_u8 i = 0;
	l_u16 result;

	/* diagnostic request have classic checksum */
	if ((pid == OPEN_LIN_DIAG_REQUEST) || (pid == OPEN_LIN_DIAG_RESPONSE) || cs_type == OPEN_LIN_CHECKSUM_TYPE_CLASSIC) {
		result = 0;
	} else {
